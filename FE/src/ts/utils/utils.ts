export const _q = (str: string) => document.querySelector(str);

export const _qa = (str: string) => document.querySelectorAll(str);
...
export const hasClass = (className, target) =>
...
export const clearClass = (classes, target) =>
  target.classList.remove(...classes);

export const addMultipleEventListener = (target, callback, ...event) =>
  event.forEach(eachEvent =>
    target.addEventListener(eachEvent, e => callback(e)),
  );
